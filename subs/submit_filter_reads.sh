...
JOB_ID=`sbatch $ARGS --export=INPUT_DIR,OUT_DIR,FILE_LIST,KMER --job-name=${JOB} -e $STDERR_DIR/${JOB}_%a.err -o $STDOUT_DIR/${JOB}_%a.out --array=1-3 ./runs/run_${JOB}.sh`
if [ "${JOB_ID}x" != "x" ]; then
    echo Job: \"$JOB_ID\"
else
